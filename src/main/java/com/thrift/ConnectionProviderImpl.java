...
public class ConnectionProviderImpl implements ConnectionProvider, InitializingBean, DisposableBean {
     ...
     * 对象缓存池
     */
    private List<ObjectPool> objectPoolList = Lists.newArrayList();
    ...
    public ObjectPool getPool() {
        try {
            ...
            return objectPoolList.get(getRandom(poolSize));
        } catch (Exception e) {
            throw new RuntimeException("error getPool()", e);
        }
    }
    ...
    public void returnCon(ObjectPool objectPool, TSocket socket) {
        try {
            // 将对象放回对象池
            if (objectPool != null && socket != null) {
    ...
    public void destroy() throws Exception {
        try {
            if (CollectionUtils.isNotEmpty(objectPoolList)) {
                for (ObjectPool objectPool : objectPoolList) {
                    objectPool.close();
                }
            }
        } catch (Exception e) {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        for (String host : hosts) {
            ...
            // 设置factory
            ThriftPoolableObjectFactory thriftPoolableObjectFactory = new ThriftPoolableObjectFactory(host, servicePort, conTimeOut);
